...
def question_list(db: Session = Depends(get_db),
    ...
    # db.close()
    # with get_db() as db:
    #_questions_list = db.query(Question).order_by(Question.id.desc()).all()
...
@router.get("/detail/{question_id}", response_model=question_schema.QuestionSchema)
def question_detail(question_id: int, db: Session = Depends(get_db)):
    question = question_crud.get_question(db, question_id=question_id)
    return question

@router.post("/create", status_code=status.HTTP_204_NO_CONTENT)
def question_create(_question_create:question_schema.QuestionCreateSchema, db: Session = Depends(get_db),
                    current_user: User = Depends(get_current_user)):
    question_crud.create_question(db, question_create=_question_create,
                                  user=current_user)

@router.put("/update", status_code=status.HTTP_204_NO_CONTENT)
def question_update(_question_update:question_schema.QuestionUpdateSchema,
                    db: Session = Depends(get_db),
                    current_user: User = Depends(get_current_user)):
    db_question = question_crud.get_question(db, question_id=_question_update.question_id)
    if not db_question:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="데이터를 찾을수가 없습니다.")
    if current_user.id != db_question.user.id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="수정 권한이 없습니다.")
    question_crud.upadte_question(db= db,
                                  db_question = db_question,
                                  question=_question_update)

@router.delete("/delete", status_code=status.HTTP_204_NO_CONTENT)
def question_delete(_question_delete:question_schema.QuestionDeleteSchema,
                    db: Session = Depends(get_db),
                    current_user: User = Depends(get_current_user)):
...
def question_vote(_question_vote:question_schema.QuestionVoteSchema,
                  db: Session = Depends(get_db),
                  current_user: User = Depends(get_current_user)):
